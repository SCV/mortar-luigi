...
class TestRedshift(unittest.TestCase):
    ...
    def test_get_column_definitions_from_pig_schema__chararray(self):
        datetime_schema = '{"fields":[{"name":"created_at","type":30,"description":"autogenerated from Pig Field Schema","schema":null}],"version":0,"sortKeys":[],"sortKeyOrders":[]}'
        datetime_redshift_columns = redshift.get_column_definitions_from_pig_schema(datetime_schema)

        self.assertEquals(1, len(datetime_redshift_columns))
        self.assertEquals(('created_at', 'timestamp'), datetime_redshift_columns[0])

    def test_get_column_definitions_from_pig_schema__datetime(self):
